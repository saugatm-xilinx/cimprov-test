    ...
    Property<uint32>& return_value)
{
    ...
    if (nv_cntx < 0)
    {
        NVContext.null = true;
        /*
         * The below code has been added as a
         * work around for Bug 86581.
         */
        if (( Type.value == NV_PART_MUMFW ) ||
            ( Type.value == NV_PART_BUNDLE ) ||
            ( Type.value == NV_PART_BUNDLE_METADATA ))
            return_value.set(UnsupportedPartition);
        else
            return_value.set(Error);
    }
    else
