...
#ifdef _WIN32
extern void ci_netif_fin_timeout_restart(ci_netif *ni, ci_tcp_state*ts) CI_HF;
#endif
...
/*! \todo It would be more efficient if the OV state were encoded into
 * the following so we can apply a mask rather than testing for each
 * type.  It'll only get worse with UDP. */
typedef enum {
  op_state_ut_error=0,
  op_state_t_any,       /* Note: t_anyov purged */
  op_state_u_any,
  op_state_t_cef,
  op_state_t_cefov,
  op_state_cos,
  op_state_coshandover,
  op_state_u_coshandover,
  op_state_unknown,     /* OS: Not sure which state we're headed to */
  op_state_unknownef,   /* Efab: Not sure which state we're headed to */

  /* add more above this comment. Note: if you add here then PLEASE
   * modify the CI_OP_STATE_IS_OS_OR_UNKNOWN() macro that follows. 
   * Ideally, please add above op_state_cos if possible/appropriate. */

  op_state_max_p1 /* last value plus 1 */
} op_state_id_t;


/* If you change the enum above, then please consider these too. */
#define CI_OP_STATE_IS_OS_OR_UNKNOWN(id)  (id >= op_state_cos)
#define CI_OP_STATE_IS_HANDOVER(id)       (id == op_state_coshandover ||  \
                                           id == op_state_u_coshandover)

#define ci_sock_get_op_state(s)           (s)->op_state_id
#define ci_sock_set_op_state(s,val)       ((s)->op_state_id)=(val)

#define ci_tcp_get_op_state(ts)           (ts)->s.op_state_id
#define ci_tcp_set_op_state(ts,val)       ((ts)->s.op_state_id)=(val)

#define ci_udp_get_op_state(us)           (us)->s.op_state_id
#define ci_udp_set_op_state(us,val)       ((us)->s.op_state_id)=(val)
...
typedef MDL ci_os_scatterlist_t;
...
#ifdef _WIN32
/*! Set the network event. NOTE that there are only a few errors that 
 * are documented to use this mechanism.  See the MSDN doc. for 
 * WSAEnumNetworkEvents() for the list. 
 * [s] is citp_sock_cmn*, [a] is API (CONNECT, RECV etc.), [e] is the
 * error value.
 */
#define sock_set_enum_event(s,a,e) \
  do { (s)->wee_enum_table[WEE_ENUM_##a]=(e); } while(0)
#define tcp_set_enum_event(t,a,e) sock_set_enum_event(&(t)->s,a,e)
#endif
...
#ifdef _WIN32
/* is state in FIN_WAIT2 - if so we timeout */
...
#endif
#if CI_CFG_CHIMNEY
extern void ci_tcp_timeout_push(ci_netif *netif, ci_tcp_state *ts) CI_HF;
#endif
...
#ifdef _WIN32
extern int ci_tcp_sendmsg_win(ci_netif* ni, ci_tcp_state* ts, int flags,
                              int to_send, ci_iovec_ptr* piov,
                              ci_ip_pkt_fmt** fill_list_out
...
# define CI_INVALID_SOCKET  ((ci_fixed_descriptor_t)(0-1))
# define CI_IS_VALID_SOCKET(fd) \
    ((ci_fixed_descriptor_t)(fd) != CI_INVALID_SOCKET)
#else
# define CI_INVALID_SOCKET  (INVALID_HANDLE_VALUE)
# define CI_IS_VALID_SOCKET(fd) ((fd) != CI_INVALID_SOCKET)
#endif

#define CI_SOCKET_ERROR    SOCKET_ERROR

#define ci_rel_os_sock_fd(fd)
...
#if CI_CFG_CHIMNEY
/* Set in ci_tcp_recvmsg_get() if a packet with the PSH flag set is
 * emptied.
 */
#define CI_MSG_PSH                  0x01000000
#endif
...
/* Call in the context of API calls that can block to determine
 * if we have to cancel the blocking call. Will return 0 if the
 * block is to continue, else < 0 and the block must be cancelled
 * (the value returned by this function is -ve error to return 
 * to the caller.) 
 */

/* Note: WinSock2 doesn't support the cancel blocking API; an app must
 * explicitly use to the WinSock 1.1 DLL.  As we don't support ver 1.1
 * we do not therefore support this. */
#define ci_win32_blocking_callback(v) 0
...
#else  
  return NI_CONF(netif).tconst_rto_initial;
#endif
}
...
#else
  return CI_TCP_TCONST_RTO_INITIAL / 1000;
#endif
}
...
ci_inline int ci_tcp_sendq_is_empty(ci_tcp_state* ts)
{ return ts->chimney.send.q == NULL; }

ci_inline int ci_tcp_sendq_not_empty(ci_tcp_state* ts)
{ return ts->chimney.send.q != NULL; }

extern void ci_tcp_sendq_drop(ci_netif* ni, ci_tcp_state* ts);
extern void ci_tcp_retrans_drop(ci_netif* ni, ci_tcp_state* ts);
...
#ifdef _WIN32
  /* ...except if the connection has been shut down, but it's not been locally
   * SHUT_RD with no further data or a subsequent remote SHUT_WR.  Grrr...
   */
  return (TCP_RX_DONE(ts) && !(TCP_RX_ERRNO(ts) == ESHUTDOWN))
    || ((tcp_urg_data(ts) & (CI_TCP_URG_IS_HERE | CI_TCP_URG_NOT_DEL) ) ?
        bytes > 1 : bytes > 0);
#else
...
#else
ci_inline int ci_tcp_tx_advertise_space(ci_tcp_state* ts) { return 0; }
#endif
...
#if !defined(__ci_driver__) && defined(_WIN32)

ci_inline ci_fd_t __ci_get_os_sock_fd(ci_fd_t fd) {
  ci_assert_nequal( fd, CI_INVALID_SOCKET );
  return fd;
}
#define ci_get_os_sock_fd(ep, fd) __ci_get_os_sock_fd((ep)->win_sock)


ci_inline void __ci_close_os_sock(ci_fd_t* fd) {
  ci_assert(fd);
  ci_assert_nequal(*fd, (ci_fd_t) CI_INVALID_SOCKET);
  *fd = (ci_fd_t) CI_INVALID_SOCKET;
}
#define ci_close_os_sock(ep) __ci_close_os_sock(&(ep)->win_sock)
                ...
                sa_family_t domain, ci_uint16 port_be16, ci_uint32 addr_be32)
{
  ...
  if (CI_LIKELY(*sa_len >= len_min)) {
    *sa_len = CI_MIN(*sa_len, len_max);
    ...
    if (*sa_len == 0) {
#ifdef __linux__  /* SunOS does not change sa_len in this case */
      *sa_len = len_min;
#endif
      return;
    }
    ...
    *sa_len = len_min;
#elif defined(_WIN32)
    /* This is an error condition & should have been trapped by the calling fn */
    ci_log("%s: space %d < sizeof struct sockaddr_in %d", __FUNCTION__,
          *sa_len, sizeof(struct sockaddr_in));
    ci_assert(0);
#else
#error Policy for update of address length, when it is shorter, not defined.
#endif
  }
}
...
#if 0
  ci_log("%s: ni %d dump pkt %d intf %d ref %d, wr %d - rd %d %d, "
         "n_buffers=%d buf_len=%d",
         __func__, (ni)->state->stack_id,
         OO_PKT_FMT(pkt), (pkt)->intf_i, (pkt)->refcount,
         (ni)->state->dump_write_i, (ni)->state->dump_read_i,
         oo_tcpdump_queue_len(ni), pkt->n_buffers, pkt->buf_len);
#endif
}
#else
