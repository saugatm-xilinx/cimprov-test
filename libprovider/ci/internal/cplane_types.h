...
/*! type for a Windows ID for an IP Interface (network adapter index) */
typedef ci_uint32 _cicpos_ifid_t;

#define _CICPOS_IFID_BAD ((_cicpos_ifid_t)(-1))

#define _CICPOS_IFID_PRINTF_FORMAT "0x%06X"
...
typedef uint32_t ci_ip_addrset_t;

#define CI_IP_ADDRSET_PRINTF_FORMAT CI_IP_PRINTF_FORMAT
#define CI_IP_ADDRSET_PRINTF_ARGS(x) CI_IP_PRINTF_ARGS(&(x)) 

#define CI_IP_ADDRSET_BAD	0xdeadbeef
#define CI_IP_ADDRSET_UNIVERSAL	0

/*! set ci_ip_addr_t to the mask implied by the ci_ip_addrset_t */
#define CI_IP_SET_MASK(ref_ipmask_he, addrset) \
        (*(ref_ipmask_he) = CI_BSWAP_BE32(addrset))
...
#define CI_IP_ADDR_SET_SUBNET(ref_subnetaddr, ref_ipaddr_he, addrset) \
        *(ref_subnetaddr) = ((*(ref_ipaddr_he) & (addrset)))
...
#define CI_IP_ADDR_SET_BCAST(ref_bcastaddr, ref_ipnet_he, addrset)  \
        *(ref_bcastaddr) = ((*(ref_ipnet_he) | ~(addrset)))
