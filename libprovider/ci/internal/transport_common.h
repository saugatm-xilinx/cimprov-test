...
#ifdef _WIN32
#define Log_E(x)      Log_FL_always(CI_UL_LOG_E, x)
#define Log_U(x)      Log_FL_always(CI_UL_LOG_U, x)
#else
...
/*! Flag set during WSAStartup to indicate whether we're running in
 *  WOW64 or not. */
extern BOOL iswow64;

/*! Check to see if WOW64 present & flag accordingly. Call once
 * during WSPStartup() - cannot safely be called during process attach. */
extern void citp_get_wow64(void);
#endif
...
# define ci_socklen INT 
# define ci_optval char
