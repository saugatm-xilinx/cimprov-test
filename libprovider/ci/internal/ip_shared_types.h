...
#ifdef _WIN32
    /* Errors causes by sendXxx() can be reported through recvXxx(). This
     * contains the +ve error value to return.  It's only valid when
     * udp_rx.length < 0.
     */
    ci_int32          rx_icmp_error;
#endif
    ci_int32          tx_length;
...
#if CI_CFG_CHIMNEY
# define CI_TCP_TCONST_RTO_INITIAL (300)      /* The RTO constants are   */
# define CI_TCP_TCONST_RTO_MIN     (300)      /* inspired by WHQL */
# define CI_TCP_TCONST_RTO_MAX     (60*1000) 
#else
...
#if CI_CFG_CHIMNEY
# define CI_TCP_TCONST_RTT_SAMPLE_MAX (30000) /* Max value for RTT sample */
  ci_iptime_t tconst_rtt_sample_max;
#endif
...
typedef struct {
  /* Args to AcceptEx */
  ci_ss_ptr  listen_ctxt CI_ALIGN(8); /*we_efab_context_t*/
  ci_ss_ptr  accept_ctxt CI_ALIGN(8); /*we_efab_context_t*/
  ci_ss_ptr  lpOutputBuffer CI_ALIGN(8);
  ci_ss_ptr  lpdwBytesReceived CI_ALIGN(8); /* int */
  ci_ss_ptr  lpOverlapped CI_ALIGN(8);
  ci_int32   dwReceiveDataLength;
  ci_int32   dwLocalAddressLength;
  ci_int32   dwRemoteAddressLength;
} ci_accept_callback_args;
#endif
...
#ifdef _WIN32
  /* Set if found to be in wrong address space - on netif lock drop, kernel
  ** will schedule APC in the right address space. */
  oo_sp                 next_addr_spc_sock_id;
  /* Use this address space id in the netif unlock callback */
  ci_addr_spc_id_t      unlock_addr_spc_id CI_ALIGN(8);
#endif
...
#define CI_SB_AFLAG_O_APPEND_BIT        7u
  /* O_NDELAY */
#ifdef __sun__
# define CI_SB_AFLAG_O_NDELAY           0x100
# define CI_SB_AFLAG_O_NDELAY_BIT       8u
#else
...
#  define CI_FD_READ_BIT     0
#  define CI_FD_WRITE_BIT    1
#  define CI_FD_OOB_BIT      2
#  define CI_FD_ACCEPT_BIT   3
#  define CI_FD_CONNECT_BIT  4
#  define CI_FD_CLOSE_BIT    5
#  define CI_FD_QOS_BIT      6
#  define CI_FD_GROUP_QOS_BIT 7
#  define CI_FD_ROUTING_INTERFACE_CHANGE_BIT 8
#  define CI_FD_ADDRESS_LIST_CHANGE_BIT       9
# else
#  define CI_FD_READ_BIT     FD_READ_BIT
#  define CI_FD_WRITE_BIT    FD_WRITE_BIT
#  define CI_FD_OOB_BIT      FD_OOB_BIT
#  define CI_FD_ACCEPT_BIT   FD_ACCEPT_BIT
#  define CI_FD_CONNECT_BIT  FD_CONNECT_BIT
#  define CI_FD_CLOSE_BIT    FD_CLOSE_BIT
#  define CI_FD_QOS_BIT      FD_QOS_BIT
#  define CI_FD_GROUP_QOS_BIT FD_GROUP_QOS_BIT
#  define CI_FD_ROUTING_INTERFACE_CHANGE_BIT FD_ROUTING_INTERFACE_CHANGE_BIT
#  define CI_FD_ADDRESS_LIST_CHANGE_BIT      FD_ADDRESS_LIST_CHANGE_BIT
# endif
/* Each entry less than WEE_ENUM_COUNT represents a ci_int16 in 
 * ci_sock_cmn::wee_enum_table[], so be sparing! */
enum {
  WEE_ENUM_READ   = CI_FD_READ_BIT,
  WEE_ENUM_RECV   = WEE_ENUM_READ,
  WEE_ENUM_WRITE  = CI_FD_WRITE_BIT,
  WEE_ENUM_SEND   = WEE_ENUM_WRITE,
  WEE_ENUM_OOB    = CI_FD_OOB_BIT,
  WEE_ENUM_ACCEPT = CI_FD_ACCEPT_BIT,
  WEE_ENUM_CONNECT= CI_FD_CONNECT_BIT,
  WEE_ENUM_CLOSE  = CI_FD_CLOSE_BIT,

#if 0
  WEE_ENUM_QOS    = CI_FD_QOS_BIT,
  WEE_ENUM_GROUP_QOS = CI_FD_GROUP_QOS_BIT,
  WEE_ENUM_ROUTING_INTERFACE_CHANGE = CI_FD_ROUTING_INTERFACE_CHANGE_BIT,
  WEE_ENUM_ADDRESS_LIST_CHANGE      = CI_FD_ADDRESS_LIST_CHANGE_BIT,
#endif

  WEE_ENUM_DEFAULT,

  /* Add new entries that have to be individually recorded above here */
  WEE_ENUM_COUNT,

  /* Entries from here down are separated out to give us scope to
   * record errors/events on an API by API basis if we wish. */
#if 1
  WEE_ENUM_QOS                      = WEE_ENUM_DEFAULT,
  WEE_ENUM_GROUP_QOS                = WEE_ENUM_DEFAULT,
  WEE_ENUM_ROUTING_INTERFACE_CHANGE = WEE_ENUM_DEFAULT,
  WEE_ENUM_ADDRESS_LIST_CHANGE      = WEE_ENUM_DEFAULT,
#endif

  WEE_ENUM_LISTEN                   = WEE_ENUM_DEFAULT,
  WEE_ENUM_BIND                     = WEE_ENUM_DEFAULT,
  WEE_ENUM_CANCELBLOCKING           = WEE_ENUM_DEFAULT,
  WEE_ENUM_IOCTL                    = WEE_ENUM_DEFAULT,
  WEE_ENUM_SELECT                   = WEE_ENUM_DEFAULT,
  WEE_ENUM_ASELECT                  = WEE_ENUM_DEFAULT,
  WEE_ENUM_ESELECT                  = WEE_ENUM_DEFAULT,
  WEE_ENUM_DISCONNECT               = WEE_ENUM_DEFAULT,
  WEE_ENUM_DUP                      = WEE_ENUM_DEFAULT,
  WEE_ENUM_ENUMNETWORKEVENTS        = WEE_ENUM_DEFAULT,
  WEE_ENUM_GETPEERNAME              = WEE_ENUM_DEFAULT,
  WEE_ENUM_GETQOS                   = WEE_ENUM_DEFAULT,
  WEE_ENUM_GETSOCKNAME              = WEE_ENUM_DEFAULT,
  WEE_ENUM_GETSOCKOPT               = WEE_ENUM_DEFAULT,
  WEE_ENUM_RECVDISCONNECT           = WEE_ENUM_DEFAULT,
  WEE_ENUM_SENDDISCONNECT           = WEE_ENUM_DEFAULT,
  WEE_ENUM_SETSOCKOPT               = WEE_ENUM_DEFAULT,
  WEE_ENUM_SHUTDOWN                 = WEE_ENUM_DEFAULT,
  WEE_ENUM_TRANSMITFILE             = WEE_ENUM_DEFAULT,
  WEE_ENUM_STARTUP                  = WEE_ENUM_DEFAULT,
  WEE_ENUM_CLEANUP                  = WEE_ENUM_DEFAULT,
};
#endif
...
#ifdef _WIN32
  op_state_id_32bits_t  op_state_id; 
  /* state index for controlling Windows WSP redirection */
#endif /* _WIN32 */
...
#ifdef _WIN32
/* IP may/not be fragmented.  The flag is remembered to retain compatibilty
 * with the sockopt API however, as per MS Platform SDK Doc (Feb 2003) we
 * can and do silently ignore it. */
#define CI_SOCK_FLAG_DF           0x01000000
 /* socket can be used for overlapped I/O operations. i.e. socket was created
  * by a call to socket() or a call to WSASocket() with WSA_FLAG_OVERLAPPED
  * specified.
  */
#define CI_SOCK_FLAG_OVERLAPPED   0x02000000
 /* This reflects !TF_REUSE_SOCKET from disconnectex. We need this
  * as socket post-disconnectex respond differently depending on that
  * flag :(
  *
  * Once set, cannot be cleared other than as part of destruction
  */
#define CI_SOCK_FLAG_CANNOT_REUSE 0x04000000
 /* socket's initial connection was via ConnectEx()
  *
  * Once set, cannot be cleared other than as part of destruction
  */
#define CI_SOCK_FLAG_CONNECTEX    0x08000000
#endif /* _WIN32 */
...
#ifdef _WIN32
  /* Disconnectex() etc have a well known side effect; shutdown() is 
   * disabled until setsockopt SOL_SOCKET:SO_UPDATE_*_CONTEXT has been
   * issued (in Server 2003; XP SP2 doesn't exhibit this).  
   * The shutdown API is disabled by the first call to connectex/acceptex
   * and is re-enabled by the first setsockopt:SOL_SOCKET:SO_UPDATE_*_CONTEXT.
   * Once enabled it cannot be disabled again.
   * CI_SB_AFLAG_CANNOT_DISABLE_SHUTDOWN can only be set.
   */
# define CI_SOCK_AFLAG_SHUTDOWN_DISABLED           0x01000000
# define CI_SOCK_AFLAG_SHUTDOWN_DISABLED_BIT               24u
# define CI_SOCK_AFLAG_CANNOT_DISABLE_SHUTDOWN     0x02000000
# define CI_SOCK_AFLAG_CANNOT_DISABLE_SHUTDOWN_BIT         25u
  /* We need to ensure that the UL address cache in each process are all
  * used in sync - the cache data themselves are in 1+ citp_socket.  Once set 
  * this flag will never be reset. */
# define CI_SOCK_AFLAG_ADDR_CACHE_ACTIVE           0x04000000
# define CI_SOCK_AFLAG_ADDR_CACHE_ACTIVE_BIT               26u

 /* These flags are used when we're in the unknown op. state, they
  * flag whether we're connecting/disconnecting and whether or not
  * we have achieved the final state.
  */
# define CI_SOCK_AFLAG_NEXT_STATE_CONNECTED        0x08000000
# define CI_SOCK_AFLAG_NEXT_STATE_CONNECTED_BIT            27u
# define CI_SOCK_AFLAG_NEXT_STATE_DISCONNECTED     0x10000000
# define CI_SOCK_AFLAG_NEXT_STATE_DISCONNECTED_BIT         28u
# define CI_SOCK_AFLAG_NEXT_STATE_MASK  \
   (CI_SOCK_AFLAG_NEXT_STATE_CONNECTED | CI_SOCK_AFLAG_NEXT_STATE_DISCONNECTED)
#endif
...
#ifdef __sun__                         /* TCP_CORK is reset to default */
#define CI_SOCK_AFLAG_TCP_INHERITED 0
#else
...
#ifdef _WIN32
  /* Error numbers recorded for a number of API (see enum earlier).  Used
  ** By EnumNetworkEvents() and must be accessible from kernel by async
  ** code. 
  */
  ci_int16 wee_enum_table[WEE_ENUM_COUNT];

/* The maximum number of triggers */
# define CI_SOCK_NUM_EVENT_TRIGGERS        3
  oo_atomic_t trigger_armed_mask[CI_SOCK_NUM_EVENT_TRIGGERS];
  /**< Event trigger currently "armed" events mask */
# define CI_SOCK_TRIGGER_ACCEPT_IDX        0
# define CI_SOCK_TRIGGER_SELECT_IDX        1
# define CI_SOCK_TRIGGER_EVENT_SELECT_IDX  2

/* The maximum number of shared events */
# define CI_SOCK_NUM_SHARED_EVENTS         2

/* Shared event index values */
# define CI_SOCK_SHARED_EVENT_ACCEPT_IDX   0
# define CI_SOCK_SHARED_EVENT_SELECT_IDX   1

/* External API visible events (WSPEventSelect(), WSPAsyncSelect(), etc.) */
# define CI_EV_READ      0x0001
# define CI_EV_WRITE     0x0002
# define CI_EV_OOB       0x0004
# define CI_EV_ACCEPT    0x0008
# define CI_EV_CONNECT   0x0010
# define CI_EV_CLOSE     0x0020
/* Avoid FD_QOS, FD_GROUP_QOS, FD_ROUTING_INTERFACE_CHANGE and
** FD_ADDRESS_LIST_CHANGE.  Next available is bit 10 (0x400).
*/

/* Internal events */
# define CI_EV_CLOSED    0x1000

/* Event groups */
# define CI_EV_NONE      0x0000
# define CI_EV_RX_WAKE   (CI_EV_READ | CI_EV_OOB | CI_EV_ACCEPT | CI_EV_CLOSE | CI_EV_CLOSED)
# define CI_EV_TX_WAKE   (CI_EV_WRITE | CI_EV_CONNECT)
# define CI_EV_EXTERNAL  0x003F
# define CI_EV_INTERNAL  0x1000
# define CI_EV_ALL       0x103F

  ci_uint32 evsel_mask;
  /*!< "Watched"/"Interested" mask for event-based select notifications */
  ci_uint32 evsel_armed_mask;
  /*!< Currently "Armed" mask for event-based select notifications */
  ci_uint32 evsel_disabled_mask;
  /*!< "Disabled" mask for event-based select notifications, i.e.
  **   CI_EV_CONNECT is not enabled for accepted sockets.
  **   CI_EV_CLOSE is not enabled after local shutdown(SD_RECEIVE or SD_BOTH)
  */
  ci_uint32 evsel_latest;
  /* The value returned for the OS backing socket the last time the OS
   * select thread determined that there was something to do for this EP */

  /* Time connection established (ticks).  Only valid on a TCP socket that
  ** has reached ESTABLISHED. */
  ci_iptime_t connected_time;

  /* Is this EP registered with the OS thread. */
  ci_uint32 os_registered;
#define CI_OS_NOT_REGISTERED 0
#define CI_OS_REGISTERED     1
#define CI_OS_BUSY           2
#endif
...
#ifdef _WIN32
#define CI_UDPF_RECV_BROADCAST  0x00000400  /*!< sockopt IP_RECEIVE_BROADCAST */
#define CI_UDPF_NO_CHECKSUM     0x00000800  /*!< sockopt UDP_NOCHECKSUM       */
/*! ioctl SIO_ENABLE_CIRCULAR_QUEUEING */
#define CI_UDPF_CIRCULAR_Q      0x00001000  
#define CI_UDPF_NO_CONNRESET    0x00002000  /*!< SIO_UDP_CONNRESET (default on) */
#endif
...
#ifdef _WIN32
  ci_ip_pkt_queue   async_tx_q;    /*!< Queue of packets waiting to be sent
                                    *   or processed & sent */
#endif
};
...
struct ci_tcp_chimney_send_queue {
  /* Lock-free with hand-off. */
  ci_uintptr_t              q_atomic;
  /* Protected by stack lock. */
  struct _NET_BUFFER_LIST** q_tail;
  struct _NET_BUFFER_LIST*  q;
  struct _NET_BUFFER*       next_nb;
  struct _MDL*              mdl;
  int                       mdl_off;
  int                       nb_bytes;
};
...
enum ci_tcp_chimney_indicate_state {
  CHIMNEY_INDICATE_NONE,
  CHIMNEY_INDICATE_DISABLED
};


struct ci_tcp_chimney_recv_queue {
  /* Lock-free with hand-off. */
  ci_uintptr_t              q_atomic;
  ci_uintptr_t              forward_nbls;
  ci_uintptr_t              return_nbls;
  /* Protected by stack lock. */
  struct _NET_BUFFER_LIST** q_tail;
  struct _NET_BUFFER_LIST*  q;
  struct _MDL*              mdl; /* current target MDL for received bytes */
  unsigned                  mdl_offset; /* bytes written to mdl so far */
  unsigned                  nb_length; /* total bytes in net buffer */
  unsigned                  nb_bytes;  /* bytes written to net buffer so far 
                                        * (including those written by NDIS) */

  enum ci_tcp_chimney_indidate_state indicate_state; /* Indication in progress? */
  unsigned indicate_size; /* Optimum size to indicate or zero if "all available" */
  unsigned indicate_nbls_pending; /* Number of NBLs we've indicated to ndis 
                                   * and not yet had returned */ 
#if CI_CFG_CHIMNEY_RX_BATCH
  struct _NET_BUFFER_LIST* complete_batch_head;
  struct _NET_BUFFER_LIST* complete_batch_tail;
  unsigned complete_batch_length;
#endif
};

#endif /* CI_CFG_CHIMNEY */
...
#if CI_CFG_CHIMNEY
  ci_iptime_t          t_retransmit_total;  /* Time spent retransmitting */
#endif
...
#if 0
  ci_iptime_t          ka_last;     /* last time keep-alive was send      */
#endif

  ci_uint32            zwin_probes; /* zero window probes counter         */
  ci_uint32            zwin_acks;   /* zero window acks counter           */
#if CI_CFG_CHIMNEY
  ci_uint32            zwin_max;    /* limit to number of probes to send  */
#endif
...
#endif
#if CI_CFG_CHIMNEY
  ci_ip_timer          push_tid;    /* TCP offload push timer             */
#endif
...
#ifdef _WIN32
#define CI_TCP_URG_IN_RECV2     0x0800 /* we're saving the last arrival */
#define CI_TCP_URG_RD_PAST      0x1000 /* have read past current OOB byte */
#define CI_TCP_URG_NOT_DEL      0x2000 /* have cleared CI_TCP_URG_IS_HERE but OOBB is not counted as delivered yet */
#endif
...
#if CI_CFG_CHIMNEY
  struct {
    struct ci_tcp_chimney_send_queue send;
    struct ci_tcp_chimney_recv_queue recv;
  } chimney;

  ci_uint32 max_snd_wnd; /* Largest window ever advertised by other end */

  struct efnic_chimney_tcp_connection_s*    chimney_connection_state;
  struct _NDIS_MINIPORT_OFFLOAD_BLOCK_LIST* chimney_terminate;
  NDIS_STATUS chimney_rx_terminate_status;
  NDIS_STATUS chimney_tx_terminate_status;
  struct _NET_BUFFER_LIST*                  chimney_abort;
  ci_uintptr_t                              chimney_deferred_orphan_next;
#endif
