...
#ifdef _WIN32
CI_CFG_OPT("EF_UL_POLL_WHILEBUSY", ul_poll_whilebusy, ci_uint32,
           "doc",
           , ul_poll_cycles,
           0, 0, 1, yesno)

  /* These two are experimental, and will probably only remain as run-time
     options for a short while. */

/* Default (0) to releasing the netif lock before the downcall as we've seen
 * problems where non-blocking downcalls will cause async callbacks to be
 * called thereby producing a deadlock */
CI_CFG_OPT("EF_SPIN_POLL_NEW", spin_poll_new, ci_uint32,
           "doc",
           , ul_poll_cycles,
           0, 0, 1, yesno)

CI_CFG_OPT("EF_SPIN_DOWNCALL_FIRST", spin_downcall_first, ci_uint32,
           "doc",
           , ul_poll_cycles,
           0, 0, 1, yesno)

CI_CFG_OPT("EF_UL_POLL_WFMO", ul_poll_wfmo, ci_uint32,
           "doc",
           , ul_poll_cycles,
           1, 0, 1, yesno)
CI_CFG_OPT("EF_UL_POLL_WFSO", ul_poll_wfso, ci_uint32,
           "doc",
           , ul_poll_cycles,
           1, 0, 1, yesno)
CI_CFG_OPT("EF_UL_POLL_GQCS", ul_poll_gqcs, ci_uint32,
           "doc",
           , ul_poll_cycles,
           1, 0, 1, yesno)
CI_CFG_OPT("EF_UL_POLL_SLEEPEX", ul_poll_sleepex, ci_uint32,
           "doc",
           , ul_poll_cycles,
           1, 0, 1, yesno)
CI_CFG_OPT("EF_UL_POLL_SOAW", ul_poll_soaw, ci_uint32,
           "doc",
           , ul_poll_cycles,
           1, 0, 1, yesno)

CI_CFG_OPT("EF_WSPLOG", wsplog, ci_uint32,
           "doc",
           wsplog,, 1, MIN, MAX, level)
CI_CFG_OPT("EF_FUNC_NOHOOKS", func_nohools, ci_uint32,
           "doc",
           , 0, 0, 1, yesno)

/* These flags define what messages will be sent to the user. In windows
 * these messages will end up in the event log and in the Linux they would
 * go to the syslog (except that this isn't currently used on linux).
 */
CI_CFG_OPT("EF_NOTICE_LEVEL_EVENT",  ci_uint8notice_level_event,  
           "doc",
           , 1, user_event_levels, 1, 0, MAX,
	   level)
#endif
