...
{
    UNUSED(CallingDev);
    UNUSED(PhysicalFunction);
    UNUSED(VirtualFunction);

    return_value.set(Error);

    if (System::target.getFuncPrivileges(PCIAddr, PrivilegeNames,
					 Privileges) == 0)
        return_value.set(OK);
    ...
    Property<uint32>& return_value)
{
    ...
    if (System::target.modifyFuncPrivileges(PCIAddr, AddedMask,
					    RemovedMask) == 0)
        return_value.set(OK);
    return INVOKE_METHOD_OK;
}
