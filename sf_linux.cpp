...
namespace solarflare 
{
    ...
    bool LinuxInterface::ifStatus() const
    {
        ...
        linuxDeviceGetAttr(sysfsPath.c_str(), "operstate",
                        buf, sizeof(buf));
    ...
    { 
        ...
        linuxDeviceGetAttr(sysfsPath.c_str(), "mtu", buf, sizeof(buf));
    ...
    { 
        ...
        linuxDeviceGetAttr(sysfsPath.c_str(), "address", buf, sizeof(buf));
        ptr = buf;

        for (i = 0; i < ETH_ALEN; i++)
        {
            mac[i] = (unsigned char) strtoul(ptr, &ptr, 16);
            ptr++;
        }
        return MACAddress(mac[0], mac[1], mac[2],
                          mac[3], mac[4], mac[5]);
    }
    ...
    bool LinuxSystem::forAllNICs(ConstElementEnumerator& en) const
    {
             ...
             device = readdir(device_dir))
        {
            ...
            LinuxNIC nic = LinuxNIC(NICNum, port_path[1], pnum,
                                    iface_path[0], iface_path[1]);
            nic.initialize(); 
            res = en.process(nic);
            ret = ret && res;
            NICNum++;
