...
/*  How to use waiter macros : */
   
  ci_wait_queue_t wq;

  {
    ci_wait_queue_waiter_init();
    int tmo;

    ci_wait_queue_waiter_pre( &wq );

    ci_wait_queue_wait_timout( &wq, ( test_race_condition ), usec, tmo);

    if (tmo == 0) /* wait timed out */ ;

    if ( ci_wait_queue_signal_pending( &wq ) ) /* rc = -ERESTARTSYS */

    ci_wait_queue_waiter_post( &wq );
  }

  {
    ...
    ci_wait_queue_wait( &wq, ( test_race_condition ) );
...
#if 0
/* These are bad as they use PG_reserved flag on the page instead
** of using VM_RESERVED on the vma.
*/
...
/*! Comment? */
ci_inline void reserve_page_range(unsigned long kva, int npages)
{
  while( npages-- ) {
    mem_map_reserve(virt_to_page(kva));
    kva += PAGE_SIZE;
  }
}
...
/*! Comment? */
ci_inline void unreserve_page_range(unsigned long kva, int npages)
{
  while( npages-- ) {
    mem_map_unreserve(virt_to_page(kva));
    kva += PAGE_SIZE;
  }
}
#endif
...
#if 0 /* ? SLP not needed */
/* pci_dev.name has been renamed to pretty_name in linux 2.6 */
#ifdef CONFIG_PCI_NAMES
# define PCIDEV_NAME(pcidev)	((pcidev)->pretty_name)
#else
# error "Not sure what to do, leaving for later; could return empty string"
#endif
#endif
