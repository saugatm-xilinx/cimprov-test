...
#ifdef _WIN32
/* we need to perform case insensitive matching on multibyte strings */
#define USE_TOMBLOWER
#endif
...
#ifdef _WIN32
  opts->notice_level_event=1;
  /*! we set this to 1, so that if a user sets EF_INTERCEPT
   *  env variable things go on by default
   *  \TODO: make windows and Linux behaviour the same
   */
  opts->ul_tcp= 1;
  opts->ul_udp= 1;
#endif
