...
/* TCP urgent data */
#ifdef _WIN32
#define TCP_URG_FMT "%s: %s%s%s%s%s%s[%u] up=%x recv2 %s tcp_rcv_usr=%u"
...
(tcp_urg_data(ts) & CI_TCP_URG_IN_RECV2) ? "CI_TCP_URG_IN_RECV2 " : "", \
(tcp_urg_data(ts) & CI_TCP_URG_RD_PAST) ? "CI_TCP_URG_RD_PAST " : "", \
(tcp_urg_data(ts) & CI_TCP_URG_NOT_DEL) ? "CI_TCP_URG_NOT_DEL " : "", \
tcp_urg_data(ts) & CI_TCP_URG_DATA_MASK, \
tcp_rcv_up(ts), \
ci_ip_queue_not_empty(&ts->recv2) ? "populated" : "empty", \
tcp_rcv_usr(ts)
#else
...
/* Netif ID, socket ID, socket state str, efab handle */
#define C_FMT "%d:%d %s/%#x"
#define C_PRI_ARGS(ctxt) (NETIF_FROM_CTXT(ctxt) ? NI_ID(NETIF_FROM_CTXT(ctxt)) : -1), \
                         (CTXT_TO_SOCK(ctxt) ? SC_FMT(CTXT_TO_SOCK(ctxt)) : -1), \
                         (CTXT_TO_SOCK(ctxt) ? ci_tcp_state_str(STATE_FROM_CTXT(ctxt)) : "<No shared state>"), \
                         (ctxt)->base.key
