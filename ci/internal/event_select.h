                                                    ...
                                                    ci_tcp_state* ts)
{
...
#if 0
  /* Windows says it behaves like this, but doesn't!
  **
  ...
  **  data has been read if this is a graceful close."
  **    -- MSDN Platform SDK documentation for WSAAsyncSelect()
  */

  /* Are we closed/shutdown for reading and now have no data left to read? */
  if ((ts->s.rx_errno & CI_SHUT_RD) && !(mask))
    mask = CI_EV_CLOSE;
#else
