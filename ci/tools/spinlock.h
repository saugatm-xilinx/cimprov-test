...
/*! Spinlock lock */
#define _ci_lock_lock(lock, file, line) ci_lock_trylock_i(&(lock)->os)

/*! Try to take out spinlock */
#define _ci_lock_trylock(lock, file, line) ci_lock_trylock_i(&(lock)->os)

/*! Spinlock unlock */
#define _ci_lock_unlock(lock, file, line) \
    do { ci_lock_unlock_i(&(lock)->os); } while (0)
...
/*! Spinlock lock */
#define _ci_irqlock_lock(lock, s, file, line) \
    do { ci_irqlock_lock_i(&(lock)->os, s); } while (0)

/*! Spinlock unlock */
#define _ci_irqlock_unlock(lock, s, file, line) \
    do { ci_irqlock_unlock_i(&(lock)->os, s); } while (0)

#else /* _WIN32 && _PREFAST_ */
